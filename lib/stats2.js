...
Stats.prototype.report = function() {
  ...
  result.timestamp = new Date(L.min(this._requestTimestamps)).toISOString();
  ...
  result.scenarioLatencies = this._scenarioLatencies;
  ...
  let now = L.max(this._requestTimestamps);
  if(now == null) {
    let now = Date.now();
  }
